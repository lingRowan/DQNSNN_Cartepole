...
class CacheDataset(torch.utils.data.Dataset):
    ...
    def to_tensor(self):
        # Ensure data is converted to tensor if not already
...
'''class CacheDataset(torch.utils.data.Dataset):
    ...
    .. note::
        ...
        Since converting images into spike-wave can be time-consuming, we recommend wrapping 
        your dataset into a :attr:`CacheDataset` object.

    Args:
        dataset (torch.utils.data.Dataset): The reference dataset object.
        cache_address (str, optional): The location of cache in the secondary memory. 
            Use :attr:`None` to cache on RAM. Default: None
    """
    def __init__(self, dataset, cache_address=None):
        self.dataset = dataset
        self.cache_address = cache_address
    ...
    def __getitem__(self, index):
        if self.cache[index] is None:
            ...
            sample, target = self.dataset[index]
            if self.cache_address is None:
                self.cache[index] = (sample, target)  # Cache it in-memory
            else:
                # Save to secondary memory
                save_path = os.path.join(self.cache_address, str(index))
                torch.save(sample, f"{save_path}.cd")
                torch.save(target, f"{save_path}.cl")
                self.cache[index] = save_path
        else:
            # Load from cache
            if self.cache_address is None:
                ...
                sample = torch.load(f"{self.cache[index]}.cd")
                target = torch.load(f"{self.cache[index]}.cl")
        return sample, target

    def reset_cache(self):
        r"""Clears the cached data. Useful when you want to change a pre-processing 
        parameter during the training process.
        """
        if self.cache_address is not None:
            for add in self.cache:
                if add:  # Ensure add is not None
                    os.remove(f"{add}.cd")
                    os.remove(f"{add}.cl")
        self.cache = [None] * len(self.dataset)
    ...
    def to_tensor(self, index=None):
        r"""Converts a specific item or the entire dataset to tensor.
        If index is provided, it converts that specific sample to tensor.

        Args:
            index (int, optional): Specific item index. If None, all items are converted.

        Returns:
            torch.Tensor: The converted sample or dataset.
        """
        if index is not None:
            # Convert specific item to tensor
            sample, target = self.__getitem__(index)
            return (torch.tensor(sample, dtype=torch.float32), torch.tensor(target, dtype=torch.long))

        # If index is None, convert the entire dataset (if applicable)
        dataset_tensor = []
        for i in range(self.__len__()):
            sample, target = self.__getitem__(i)
            dataset_tensor.append((torch.tensor(sample, dtype=torch.float32), torch.tensor(target, dtype=torch.long)))
