...
class CustomNavigationEnv(gym.Env):
    def __init__(self):
        ...
        self.trees = self.generate_fixed_tree()
    ...
    def generate_fixed_tree(self):
        fixed_tree = [(11,9), (0,9), (9,11),(0,9), (6,6), (8,8)]
        selected_position = fixed_tree[np.random.choice(len(fixed_tree))]

        return [selected_position]
    
    '''def generate_random_blocks(self, num_blocks=1, min_distance=2):
    ...
    # Calculate the next position based on the action
     ...
     #tree reward
        if self.current_position in self.trees:
            if not self.tree_visited:
                current_reward += 2  # Give reward only on first visit
    ...
    def get_state(self):
        ...
        agent_x, agent_y = self.current_position
        screen[agent_x, agent_y] = [255, 0, 0]  # Red for agent

        goal_x, goal_y = self.goal_position
        screen[goal_x, goal_y] = [0, 255, 0]  # Green for goal

    # Replace blocks with trees
        for tree in self.trees:
            tree_x, tree_y = tree
            screen[tree_x, tree_y] = [0, 255, 0]  # green for trees
       ...
       # action = select_action(state)  # Get the action from the SNN
        ...
        # Append the screen state to the frames
            ...
            if done:
                ...
                different_steps = steps_no - optimal_steps
            ...
            # Calculate and log accuracy
                ...
                with open("Analysis.txt", "a") as f:
                       f.write(f"number of tree 1 at positions  {env.trees} at Config {config_idx} at episode {i_episode} with total number of rewards: {total_reward}, total steps{steps_no}, the average reward per action: {Average_reward}, and difference between optimal steps and model steps are {different_steps}\n")
