    ...
    # Check if the next position hits a block (wall)
     if tuple(next_position) in self.blocks:
        ...
        if tuple(self.current_position) in self.trees:
            current_reward = 0.2  # Reward for reaching a tree
...
class SNN(nn.Module):
    def __init__(self, input_channels, features_per_class, number_of_classes):
        ...
        self.stdp = STDP(conv_layer=self.conv, learning_rate=(0.009, -0.05), use_stabilizer=True, lower_bound=0, upper_bound=1)
        self.anti_stdp = STDP(conv_layer=self.conv, learning_rate=(-0.05, 0.009), use_stabilizer=True, lower_bound=0, upper_bound=1)
